...
public class Login extends BaseComponent {
  ...
  public void alertVerification(String alertText) {
    ...
    if (isAlertPresent(2)) {
      RESULT.PASS("Alert Message exist after entering invalid credentials", false,
          ScreenshotType.browser);
      // Verify the Alert text
      verifyAlertMsg_AlertAction_PartialText(AlertAction.Accept, alertText);
      pause(ERPBaseSuite.waitLow);
    } else {
      RESULT.FAIL("Failed to check the text on the alert and invalid text", true,
          ScreenshotType.browser);
    }
  }
  ...
  public Dashboard login(String userName, String password) {
    ...
    waitForElement(Login_OR.username, 20, WaitType.visibilityOfElementLocated);
